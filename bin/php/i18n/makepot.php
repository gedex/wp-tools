...
if ( __FILE__ == $included_files[0] ) {
	$makepot = new MakePOT;
	if ( ( in_array( count( $argv ), array( 3, 4, 5 ) ) ) && in_array( $method = str_replace( '-', '_', $argv[1] ), get_class_methods( $makepot ) ) ) {
		$res = call_user_func(
		...
		$usage  = "Usage: php makepot.php PROJECT DIRECTORY [OUTPUT] [TEXTDOMAIN]\n\n";
		$usage .= "Generate POT file from the files in DIRECTORY [OUTPUT] [TEXTDOMAIN]\n";
